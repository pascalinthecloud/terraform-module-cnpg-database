...
resource "kubernetes_manifest" "cluster" {
  ...
  manifest = {
    ...
    spec = merge({
      ...
      var.cluster.image_name != null ? { imageName = var.cluster.image_name } : {},
      {
        ...
        postgresql = {
          ...
          }, var.cluster.shared_preload_libraries != null ? { shared_preload_libraries = var.cluster.shared_preload_libraries } : {})
        }
        ...
        bootstrap = {
          ...
          }, length(var.cluster.bootstrap_post_init_sql) > 0 ? { postInitSQL = var.cluster.bootstrap_post_init_sql } : {})
        }
